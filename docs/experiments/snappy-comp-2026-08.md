...
Verdict (final): NEGATIVE on both architectures (−2…−6 % gfx942, −1…−4 % gfx1100) — REVERTED. The double-buffered match state costs registers and LDS traffic on every round, while the barrier it removes is one `s_barrier` per round — cheap next to the match search itself — so there was nothing to win.
